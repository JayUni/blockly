...
Blockly.Skoolbot['lists_create_empty'] = function(block) {
  ...
  return ['{}', Blockly.Skoolbot.ORDER_ATOMIC];
};
...
Blockly.Skoolbot['lists_create_with'] = function(block) {
  ...
  for (var i = 0; i < block.itemCount_; i++) {
    elements[i] = Blockly.Skoolbot.valueToCode(block, 'ADD' + i,
        Blockly.Skoolbot.ORDER_NONE) || 'None';
  }
...
Blockly.Skoolbot['lists_repeat'] = function(block) {
  ...
  var functionName = Blockly.Skoolbot.provideFunction_(
      ...
      ['function ' + Blockly.Skoolbot.FUNCTION_NAME_PLACEHOLDER_ + '(item, count)',
       '  local t = {}',
       '  for i = 1, count do',
       '    table.insert(t, item)',
       '  end',
...
Blockly.Skoolbot['lists_length'] = function(block) {
  ...
  return ['#' + list, Blockly.Skoolbot.ORDER_UNARY];
};
...
Blockly.Skoolbot['lists_indexOf'] = function(block) {
  ...
  if (block.getFieldValue('END') == 'FIRST') {
    var functionName = Blockly.Skoolbot.provideFunction_(
        'first_index',
        ['function ' + Blockly.Skoolbot.FUNCTION_NAME_PLACEHOLDER_ + '(t, elem)',
 ...
 * @private
 */
Blockly.Skoolbot.lists.getIndex_ = function(listName, where, opt_at) {
  if (where == 'FIRST') {
    return '1';
...
Blockly.Skoolbot['lists_getIndex'] = function(block) {
  ...
  var list = Blockly.Skoolbot.valueToCode(block, 'VALUE', Blockly.Skoolbot.ORDER_HIGH) ||
      '{}';
  var getIndex_ = Blockly.Skoolbot.lists.getIndex_;
  ...
  if ((where == 'LAST' || where == 'FROM_END' || where == 'RANDOM') &&
    ...
    if (mode == 'REMOVE') {
      ...
      var atOrder = (where == 'FROM_END') ? Blockly.Skoolbot.ORDER_ADDITIVE :
          Blockly.Skoolbot.ORDER_NONE;
      var at = Blockly.Skoolbot.valueToCode(block, 'AT', atOrder) || '1';
      var listVar = Blockly.Skoolbot.variableDB_.getDistinctName(
      ...
      var at = Blockly.Skoolbot.valueToCode(block, 'AT', Blockly.Skoolbot.ORDER_NONE) ||
          '1';
      if (mode == 'GET') {
        var functionName = Blockly.Skoolbot.provideFunction_(
    ...
    var atOrder = (mode == 'GET' && where == 'FROM_END') ?
        Blockly.Skoolbot.ORDER_ADDITIVE : Blockly.Skoolbot.ORDER_NONE;
    var at = Blockly.Skoolbot.valueToCode(block, 'AT', atOrder) || '1';
...
Blockly.Skoolbot['lists_setIndex'] = function(block) {
  ...
  var at = Blockly.Skoolbot.valueToCode(block, 'AT',
      Blockly.Skoolbot.ORDER_ADDITIVE) || '1';
...
Blockly.Skoolbot['lists_getSublist'] = function(block) {
  ...
  var functionName = Blockly.Skoolbot.provideFunction_(
      ...
      ['function ' + Blockly.Skoolbot.FUNCTION_NAME_PLACEHOLDER_ + '(source' +
          // The value for 'FROM_END' and'FROM_START' depends on `at` so
          // we add it as a parameter.
...
Blockly.Skoolbot['lists_reverse'] = function(block) {
  ...
  var functionName = Blockly.Skoolbot.provideFunction_(
      'list_reverse',
      ['function ' + Blockly.Skoolbot.FUNCTION_NAME_PLACEHOLDER_ + '(input)',
       '  local reversed = {}',
       '  for i = #input, 1, -1 do',
